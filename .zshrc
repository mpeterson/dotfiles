...
# Start tmux if not already in tmux (skip for root and SSH).
...
# enabled hosts.
zstyle ':z4h:ssh:*' send-extra-files '~/.env.zsh' '~/.zshrc.d' '~/.config/tmux'
...
if [[ -d "${ZDOTDIR:-$HOME}/.zshrc.d" ]]; then
  for file in "${ZDOTDIR:-$HOME}/.zshrc.d/"*.aliases; do
      z4h source -c "$file"
  done
fi
